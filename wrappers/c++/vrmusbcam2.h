// ==============================================================================================
/// @file vrmusbcam2.h                                                   VRmUsbCam C API v3.3.2.0
...
/// periodically, at least once every 5 seconds, and handle the PnP events
/// using static callback handlers, see VRmUsbCamRegisterStaticCallback()
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamUpdateDeviceKeyListEx(VRmBOOL f_local, VRmBOOL f_usb, VRmBOOL f_ethernet);

/// trigger a full rescan of the device key list on the next call to VRmUsbCamUpdateDeviceKeyList(Ex)
/// usage: use this function immediately followed by a call to VRmUsbCamUpdateDeviceKeyList(Ex)
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamResetDeviceKeyList(void);
...
/// create image container with given format, buffer, pitch and buffer size.
/// use this function to create images from RLE or BAYER images
/// NOTE: free image using VRmUsbCamFreeImage() but you still have to deallocate fp_buffer YOURSELF!
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamSetImageEx(VRmImage** fpp_image, VRmImageFormat f_image_format, VRmBYTE* fp_buffer, VRmDWORD f_pitch, VRmDWORD f_buffer_size);
...
/// enum for user LUT formats
typedef VRM_ENUM _VRmUserLutFormat
{
	/// no user LUT, use this to unset user LUT
	VRM_USER_LUT_NONE=0,
	/// LUT 8bit->16bit, size=256 VRmWORDs
	VRM_USER_LUT_8_TO_16,
	/// LUT 10bit->16bit, size=1024 VRmWORDs
	VRM_USER_LUT_10_TO_16,
	/// LUT 16bit->16bit, size=2^16 VRmWORDs
	VRM_USER_LUT_16_TO_16,
	VRM_USER_LUT_END
} VRmUserLutFormat;
...
///     fpp_lut: LUT buffer to write to (use VRmUsbCamFreeUserLut to free the memory when you are done)
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamGetUserLut(VRmUsbCamDevice f_device, VRmDWORD f_port, VRmDWORD f_channel, VRmUserLutFormat f_format,
												 VRmWORD** fpp_lut);

/// free user lut received by VRmUsbCamGetUserLut()
/// parameters 
///     fpp_lut: LUT buffer received by VRmUsbCamGetUserLut()
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamFreeUserLut(VRmWORD** fpp_lut);
...
///     fpp_lut: LUT buffer, length and content have to match the format and number of channels
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamSetUserLut(VRmUsbCamDevice f_device, VRmDWORD f_port, VRmDWORD f_channel,
												 VRmUserLutFormat f_format, const VRmWORD* fp_lut);
...
///     f_device: pointer to device
///     f_port: affected sensor port, use 1 for single sensor cameras
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamUserLutCommit(VRmUsbCamDevice f_device, VRmDWORD f_port);

typedef VRM_ENUM _VRmEventReturnType
{
	/// an error occured
	VRM_EVENT_ERROR = 0,
	/// a timeout occurred
...
VRM_EXTERN VRmRetVal VRM_API VRmUsbCamWaitForStrobe(VRmUsbCamDevice f_device, int f_timeout_us, VRmEventReturnType* fp_event_type);
